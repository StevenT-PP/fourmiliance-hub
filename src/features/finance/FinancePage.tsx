    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-pulse">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-white rounded-xl border border-fourmiliance-border border-l-4 border-l-fourmiliance-border p-5">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-5 h-5 bg-fourmiliance-border rounded" />
                <div className="h-3 w-24 bg-fourmiliance-border rounded" />
              </div>
              <div className="h-7 w-28 bg-fourmiliance-border rounded mb-1" />
              <div className="h-3 w-20 bg-fourmiliance-border rounded" />
            </div>
          ))}
        </div>
        <div className="bg-white rounded-xl border border-fourmiliance-border p-6 animate-pulse">
          <div className="h-4 w-48 bg-fourmiliance-border rounded mb-6" />
          <div className="flex items-end gap-1.5 h-36">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full bg-fourmiliance-border rounded-t" style={{ height: `${20 + Math.random() * 60}%` }} />
                <div className="h-2 w-6 bg-fourmiliance-border rounded" />
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KpiCard
          icon={<TrendingUp className="w-5 h-5 text-fourmiliance-mid" aria-hidden="true" />}
        ...
        />
        <KpiCard
          icon={<Clock className="w-5 h-5 text-fourmiliance-ocre" aria-hidden="true" />}
          label="En attente"
        ...
        />
        <KpiCard
          icon={<Percent className="w-5 h-5 text-fourmiliance-light" aria-hidden="true" />}
      ...
      <div className="bg-white rounded-xl border border-fourmiliance-border p-6">
        ...
        {maxBar === 0 ? (
          ...
          <div role="img" aria-label="Graphique du chiffre d'affaires mensuel sur 12 mois">
              ...
              {chartBars.map(bar => {
                ...
                return (
                  <div key={bar.label} className="flex-1 flex flex-col items-center gap-1 group relative">
                    ...
                    />
                    <span className="text-[10px] text-fourmiliance-ghost text-center leading-none">
                      {bar.label}
                    </span>
                  </div>
                )
            ...
            <div className="flex items-center gap-4 mt-3 pt-3 border-t border-fourmiliance-track">
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-sm bg-fourmiliance-mid/40" />
                <span className="text-[10px] text-fourmiliance-ghost">Mois précédents</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-sm bg-fourmiliance-ocre" />
                <span className="text-[10px] text-fourmiliance-ghost">Mois en cours</span>
              </div>
            </div>
          </div>
        )}
...
function KpiCard({
  ...
  value: string
  sub: string
  accent: 'green' | 'warm' | 'light'
}) {
  const border = {
    green: 'border-l-fourmiliance-mid',
    warm:  'border-l-fourmiliance-ocre',
    light: 'border-l-fourmiliance-light',
  }[accent]

  return (
    <div className={`bg-white rounded-xl border border-fourmiliance-border border-l-4 ${border} p-5`}>
      <div className="flex items-center gap-2 mb-3">
        {icon}
      ...
      <p className="text-xs text-fourmiliance-ghost mt-1">{sub}</p>
    </div>
  )
}
